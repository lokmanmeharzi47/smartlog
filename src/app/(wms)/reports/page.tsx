      ...
      <div className="px-4 md:px-8 py-5 space-y-5 max-w-[1600px] mx-auto fade-in">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-3">
          <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-4">
            ...
            <p className="text-xl sm:text-2xl font-bold text-primary font-mono">{data.totalStockValue.toLocaleString('fr-FR')} DZD</p>
          </div>
          ...
          <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-4">
            ...
            <p className="text-xl sm:text-2xl font-bold text-primary font-mono">{data.coverageRate} %</p>
          ...
          <div className="bg-white border border-slate-200 shadow-sm rounded-2xl p-4">
            ...
            <p className="text-xl sm:text-2xl font-bold text-primary font-mono">{data.rotationRate}</p>
          ...
          <div className={`bg-white border rounded-2xl shadow-sm p-4 ${data.stockHealthScore > 80 ? 'border-emerald-200' : data.stockHealthScore > 50 ? 'border-orange-200' : 'border-red-200'}`}>
            ...
            <p className={`text-xl sm:text-2xl font-bold font-mono ${data.stockHealthScore > 80 ? 'text-emerald-500' : data.stockHealthScore > 50 ? 'text-orange-500' : 'text-red-500'}`}>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              ...
              <span className="text-slate-500 text-xs font-semibold">Volume Total</span>
            </div>
            <span className="text-primary font-bold font-mono text-sm">{data.totalStock.toLocaleString('fr-FR')} u</span>
          </div>
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
              <ShieldAlert className="w-4 h-4 text-red-400" />
              <span className="text-slate-500 text-xs font-semibold">En Rupture</span>
            </div>
            <span className="text-red-500 font-bold font-mono text-sm">{data.criticalProducts}</span>
          </div>
          <div className="bg-slate-50 border border-slate-200 rounded-xl p-3.5 flex items-center justify-between">
            <div className="flex items-center gap-2.5">
            ...
            <span className="text-secondary font-bold font-mono text-sm">{data.todayMovements}</span>
          </div>
        </div>
